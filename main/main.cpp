...
void gpio_pin_init() {
    ...
    // Разрешаем прерывания по спаду. Тоесть мы должны подать минус на пин чтобы сработало прерывание 
    ...
    // Включаем подтягивающий резистор потому что кнопка подключена к GND
